...
export default function ConsultaRecebimentos() {
  ...
  const totalRecebido = filtrados.reduce((s, c) => s + (c.valor_pagamento || 0), 0)
  const totalDesconto = filtrados.reduce((s, c) => s + (c.valor_desconto || 0), 0)

  const porForma = {}
  filtrados.forEach((c) => {
    const f = c.forma_recebimento || 'Não informado'
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, margin: 20, marginBottom: 0, padding: '16px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <div style={{ background: 'var(--gray-50)', borderRadius: 8, padding: '10px 14px', border: '1px solid var(--border-md)', minWidth: 140 }}>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Recebido no período</div>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#22863A' }}>{fmt(totalRecebido)}</div>
          </div>
          <div style={{ background: 'var(--gray-50)', borderRadius: 8, padding: '10px 14px', border: '1px solid var(--border-md)', minWidth: 140 }}>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Descontos concedidos</div>
            <div style={{ fontSize: 16, fontWeight: 600, color: '#C53030' }}>{fmt(totalDesconto)}</div>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Data pagto.', 'Documento', 'Cliente', 'Forma', 'Valor doc.', 'Desconto', 'Valor recebido', 'Usuário'].map((h) => (
                    <th key={h} style={{ padding: '9px 14px', fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textAlign: ['Valor doc.', 'Desconto', 'Valor recebido'].includes(h) ? 'right' : 'left', background: 'var(--gray-50)', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtrados.map((c) => (
                  <tr key={c.id} style={{ borderBottom: '1px solid var(--border)' }}
                    ...
                    <td style={{ padding: '10px 14px', fontSize: 13, fontWeight: 700, textAlign: 'right', color: '#22863A' }}>{fmt(c.valor_pagamento)}</td>
              ...
              <tfoot>
                <tr style={{ background: 'var(--gray-50)', borderTop: '2px solid var(--border)', fontWeight: 700 }}>
                  <td colSpan={5} style={{ padding: '10px 14px', fontSize: 12 }}>TOTAL — {filtrados.length} recebimento(s)</td>
                  <td style={{ padding: '10px 14px', fontSize: 13, textAlign: 'right' }}>{fmt(totalDesconto)}</td>
                  <td style={{ padding: '10px 14px', fontSize: 13, textAlign: 'right', color: '#22863A' }}>{fmt(totalRecebido)}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
