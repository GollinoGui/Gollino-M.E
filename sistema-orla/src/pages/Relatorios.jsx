...
function RelContasReceber() {
  ...
  function agruparPorCliente(lista) {
    ...
    for (const c of lista) {
      const nome = pessoaLabel(c.nome_cliente, c.codigo_cliente)
      ;(itensPorNome[nome] ||= []).push(c)
    }
    return Object.keys(itensPorNome)
      .sort((a, b) => a.localeCompare(b, 'pt-BR'))
      .map((nome) => ({
        nome,
        itens: itensPorNome[nome].sort((a, b) =>
  ...
  function exportarExcel() {
    ...
    if (agruparCliente) {
      linhas = []
      for (const { nome, itens } of agruparPorCliente(contas)) {
        ...
        linhas.push({
          Documento: '', Seq: '', Cliente: `Subtotal - ${nome}`, Vencimento: '',
          'Valor (R$)': subVal.toFixed(2).replace('.', ','),
          'Pago (R$)': subPago.toFixed(2).replace('.', ','),
          'Em Aberto (R$)': (subVal - subPago).toFixed(2).replace('.', ','),
          Situação: '',
        })
      }
    } else {
      ...
      linhas = porVencimento.map((c) => linhaDe(c, pessoaLabel(c.nome_cliente, c.codigo_cliente)))
    }
    ...
    exportarCSV(linhas, agruparCliente ? 'contas_receber_por_cliente' : 'contas_receber')
  }
  ...
  async function gerarRelatorioPDF() {
    ...
    if (agruparCliente) {
      ...
      const grupos = agruparPorCliente(contas)
      const secoes = grupos.map(({ nome, itens }) => {
        const subVal = itens.reduce((s, c) => s + (c.valor_docto || 0), 0)
        const subPago = itens.reduce((s, c) => s + (c.valor_pagamento || 0), 0)
        const subAberto = subVal - subPago
        return {
          titulo: nome,
          colunas: colunasGrupo,
          linhas: itens,
          montarLinha: montarLinhaGrupo,
          montarTotal: () =>
            `<td>Subtotal</td><td></td><td class="num">${fmtMoedaBR(subVal)}</td><td class="num">${fmtMoedaBR(subPago)}</td><td class="num">${fmtMoedaBR(subAberto)}</td><td></td>`,
        }
      })
      html = gerarHtmlSecoes({
        empresa,
        titulo: 'Contas a Receber por Cliente',
        subtitulo: `${contas.length} parcela(s) — ${grupos.length} cliente(s) — Em aberto: ${fmtMoedaBR(totalAberto)}`,
        secoes,
      })
    } else {
      const porVencimento = [...contas].sort((a, b) =>
    ...
    await gerarPdfRelatorio(html, agruparCliente ? 'contas_receber_por_cliente' : 'contas_receber')
  }
  ...
  return (
    <div style={{ padding: 20, overflowY: 'auto', height: '100%' }}>
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'flex-end' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, height: 34, fontSize: 12, color: 'var(--text-secondary)', cursor: 'pointer' }}>
          <input
            type='checkbox'
            checked={agruparCliente}
            onChange={(e) => setAgruparCliente(e.target.checked)}
            style={{ width: 15, height: 15, cursor: 'pointer' }}
          />
          Agrupar por cliente (Excel/PDF)
        </label>
      </div>
      {loading ? (
        <Carregando />
