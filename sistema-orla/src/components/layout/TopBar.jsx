...
import {
  ShoppingCart, FileText, Undo2, Wallet, Users, Package,
  BarChart2, Settings, Wrench, ChevronDown, X,
  FolderOpen, TrendingUp, FileBarChart, Cog, Database,
  DollarSign, BookOpen, ClipboardList, RotateCcw,
  Banknote, Receipt, CreditCard, ArrowUpCircle,
  ArrowDownCircle, Search, AlertCircle
} from 'lucide-react'
...
const menus = [
  {
    ...
    items: [
      ...
      { id: 'contador-dinheiro',label: 'Contador de dinheiro', icon: DollarSign },
  ...
  },
  {
    ...
    label: 'Estoque',
    items: [
      { id: 'entrada-mercadoria',  label: 'Entrada de mercadoria',   icon: ArrowDownCircle },
      { id: 'pedido-compra',       label: 'Pedido de compra',        icon: ClipboardList },
      { id: 'saida-mercadoria',    label: 'Saída de mercadoria',     icon: ArrowUpCircle },
      { id: 'acerto-estoque',      label: 'Acerto de estoque',       icon: Settings },
      { id: 'contagem-estoque',    label: 'Contagem de estoque',     icon: ClipboardList },
      { id: 'movimento-produto',   label: 'Movimento de produto',    icon: TrendingUp },
      { id: 'consulta-reajustes',  label: 'Consulta de reajustes',   icon: Search },
    ],
  },
  {
    id: 'financeiro',
    ...
    items: [
      ...
      { id: 'consulta-recebimentos',label: 'Consulta de recebimentos',icon: Search },
      { id: 'consulta-pagamentos',  label: 'Consulta de pagamentos',  icon: Search },
    ],
  },
  {
    id: 'fiscal',
    ...
    items: [
      ...
      { id: 'manifestacao',           label: 'Manifestação destinatário', icon: AlertCircle },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'rel-entradas',       label: 'Relatórios entradas',        icon: ArrowDownCircle },
      { id: 'rel-produtos',       label: 'Relatórios produtos',        icon: Package },
      { id: 'rel-contas-pagar',   label: 'Relatórios contas a pagar',  icon: DollarSign },
      { id: 'rel-contas-receber', label: 'Relatórios contas a receber',icon: Wallet },
...
export default function TopBar({ activePage, onNavigate, caixaAberto }) {
  ...
  return (
    <div style={{
      height: 46, background: 'var(--blue-700)',
  ...
  padding: '0 20px', borderRight: '1px solid rgba(255,255,255,0.12)',
  flexShrink: 0, cursor: 'pointer',
}}>
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3, width: 26, height: 26 }}>
    {[0,1,2,3].map(i => (
      <div key={i} style={{ background: i%2===0 ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.5)', borderRadius: 3 }} />
    ))}
  </div>
  <span style={{ color: '#fff', fontWeight: 600, fontSize: 14, letterSpacing: '-0.2px' }}>
    Gollino M.E
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', flex: 1, overflowX: 'visible' }}>
        {menus.map(menu => {
            ...
            <div key={menu.id} style={{ position: 'relative', flexShrink: 0 }}>
              <button
                ...
                style={{
                  ...
                  color: isOpen || hasActive ? '#fff' : 'rgba(255,255,255,0.72)',
                  background: isOpen ? 'rgba(255,255,255,0.15)' : hasActive ? 'rgba(255,255,255,0.1)' : 'transparent',
                  borderRight: '1px solid rgba(255,255,255,0.07)',
                  fontSize: 13, fontWeight: isOpen || hasActive ? 500 : 400,
              ...
              {isOpen && (
                <div style={{
                  ...
                  {menu.items.map((item, idx) => {
                    ...
                    return (
                      <button key={item.id} onClick={() => handleItem(item.id)} style={{
                        width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                        padding: '9px 14px', fontSize: 13,
                        color: isActive ? 'var(--blue-700)' : 'var(--text-primary)',
                        background: isActive ? 'var(--blue-50)' : 'transparent',
                        borderBottom: idx < menu.items.length - 1 ? '1px solid var(--border)' : 'none',
                      ...
                      >
                        <Icon size={14} style={{ color: isActive ? 'var(--blue-600)' : 'var(--text-muted)', flexShrink: 0 }} />
                        {item.label}
                      </button>
                    )
                  })}
      ...
      <div style={{
        ...
        padding: '0 16px', flexShrink: 0, borderLeft: '1px solid rgba(255,255,255,0.12)',
      }}>
        <div style={{
          ...
          <div style={{ width: 7, height: 7, borderRadius: '50%', background: caixaAberto ? '#4ade80' : '#f87171' }} />
          {caixaAberto ? 'Caixa aberto' : 'Caixa fechado'}
        </div>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>Secretaria</span>
      </div>
    </div>
  )
}