      ...
      try {
        ...
        ;(clientes || []).slice(0, 4).forEach((c) =>
          res.push({
            ...
            id: 'clientes',
            label: c.nome,
            desc: `#${c.codigo}${c.cpf_cnpj ? ` · ${c.cpf_cnpj}` : ''}`,
            icon: Users,
            categoria: 'Clientes',
            dados: c,
          }),
        ...
        ;(produtos || []).slice(0, 4).forEach((p) =>
          res.push({
            ...
            id: 'produtos',
            label: p.descricao,
            desc: `#${p.codigo} · ${fmt(p.preco_venda_vista)} · Estoque: ${fmtQtd(p.estoque_atual ?? 0, p.unidade)}`,
            icon: Package,
            categoria: 'Produtos',
            dados: p,
          }),
        ...
        ;(contas || [])
          ...
          .forEach((c) =>
            res.push({
              ...
              desc: `#${c.codigo_cliente} · Doc. ${c.numero_docto} · ${fmt(c.valor_em_aberto)} em aberto`,
              icon: Wallet,
